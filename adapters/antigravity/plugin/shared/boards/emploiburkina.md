# Board adapter — Emploi Burkina (Burkina Faso): reopened by the 2026-09-07 doctrine, and the transport refuses the client with a static 403 — and served to a browser

<!-- verified: 2026-09-13 -->

<!-- hosts: www.emploiburkina.com, emploiburkina.com -->
<!-- script: none -->
<!-- countries: BF -->
<!-- content: measured · **73 distinct advertisement addresses** over `/recherche-jobs-burkina-faso?page=0…` (25 + 25 + 23; the next page empty), read from a connected browser tab, **against «73 Offres d'emploi trouvées» stated by the page — equal**; the pager is zero-based, as on every AfricaWork host read since `ghanajob.md`; the rules as on the 12th — `ClaudeBot` refused, `*` open, the 2026-09-07 doctrine and #230 · 2026-09-13 -->
<!-- witness: the page's own «73 Offres d'emploi trouvées», read on every page of the walk and printed beside the distinct count («73 emitted, site states 73 — equal») · 2026-09-13 -->
<!-- route: browser · 73 · 2026-09-13 -->

**Measured 2026-09-12 at 11:18:58Z UTC for #233, lot 4 — a measurement of the
...
```
GET https://www.emploiburkina.com/                                 403, 25 B, md5 9ccabba20b9f    (11:18:58Z)
GET https://www.emploiburkina.com/                                 403, 25 B, md5 9ccabba20b9f    (second fetch)
GET https://www.emploiburkina.com/recherche-jobs-burkina-faso     403, 25 B, md5 9ccabba20b9f    (11:21:12Z)
GET https://www.emploiburkina.com/recherche-jobs-burkina-faso     403, 25 B, md5 9ccabba20b9f    (second fetch)
```
...
## 2026-09-13 10:41 UTC — the browser route, MEASURED (#222): the 403 is for the declared client alone

One Claude-in-Chrome tab, the guard on `/recherche-jobs-burkina-faso` first (`*` open, certain).
**No challenge, no interstitial** — borne 0 held: the 25-byte 403 above goes
to the declared HTTP client and to nobody else.

```
navigate https://www.emploiburkina.com/recherche-jobs-burkina-faso      200 — «73 Offres d'emploi trouvées» · 25 cards a page · pager zero-based (the link labelled «2» is ?page=1)
fetch ?page=0, 1, 2 …  (25 + 25 + 23)        **73 distinct /offre-emploi-burkina-faso/<slug>-<id>** · the next page → 0 cards   10:41 UTC
                                       **73 emitted, site states 73 — equal.**
GET /offre-emploi-burkina-faso/commercial-ouagadougou-321012                  200 — a JobPosting in JSON-LD (datePosted 2026-09-11T16:10:43+01:00), the JSON rejected by a strict parser (control characters) — read tolerantly, as `ghanajob.md` says
```

**The procedure is `ghanajob.md`'s six steps with this host's two paths**
— `/recherche-jobs-burkina-faso` for the listing, `/offre-emploi-burkina-faso/<slug>-<id>` for the
...
- **No script, no configuration.** A user with a URL from this host can hand
  ...
  established here.
- **AfricaWork franchise** (the same 1 836-byte managed block as `www.emploibenin.com`, `www.emploi.cm`, `www.emploisenegal.com`) and the static default, as five of seven in lot 2 — **one read of this host, dated**: under this very file two lot-2 members answer a challenge instead.
