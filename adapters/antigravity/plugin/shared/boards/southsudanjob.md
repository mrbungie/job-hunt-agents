# Board adapter — SouthSudanJob (South Sudan): reopened by the 2026-09-07 doctrine, and the transport refuses the client with a static 403 — and served to a browser

<!-- verified: 2026-09-13 -->

<!-- hosts: www.southsudanjob.com, southsudanjob.com -->
<!-- script: none -->
<!-- countries: SS -->
<!-- content: measured · **97 distinct advertisement addresses** over `/job-vacancies-search-south-sudan?page=0…` (25 + 25 + 25 + 22; the next page empty), read from a connected browser tab, **against «97 Job ads found» stated by the page — equal**; the pager is zero-based, as on every AfricaWork host read since `ghanajob.md`; the rules as on the 12th — `ClaudeBot` refused, `*` open, the 2026-09-07 doctrine and #230 · 2026-09-13 -->
<!-- witness: the page's own «97 Job ads found», read on every page of the walk and printed beside the distinct count («97 emitted, site states 97 — equal») · 2026-09-13 -->
<!-- route: browser · 97 · 2026-09-13 -->

**Measured 2026-09-13 at 10:22:12Z UTC for #233, lot 8 — a measurement of the
...
```
GET https://www.southsudanjob.com/                             403, 25 B, md5 9ccabba20b9f    (10:22:12Z)
GET https://www.southsudanjob.com/                             403, 25 B, md5 9ccabba20b9f    (second fetch)
GET https://www.southsudanjob.com/search-jobs-south-sudan     403, 25 B, md5 9ccabba20b9f    (10:23:20Z)
GET https://www.southsudanjob.com/search-jobs-south-sudan     403, 25 B, md5 9ccabba20b9f    (second fetch)
```
...
One Claude-in-Chrome tab, the guard on `/job-vacancies-search-south-sudan` first (`*` open, certain).
**No challenge, no interstitial** — borne 0 held: the 25-byte 403 above goes
to the declared HTTP client and to nobody else.

```
navigate https://www.southsudanjob.com/job-vacancies-search-south-sudan      200 — «97 Job ads found» · 25 cards a page · pager zero-based (the link labelled «2» is ?page=1)
fetch ?page=0, 1, 2 …  (25 + 25 + 25 + 22)        **97 distinct /job-vacancies-south-sudan/<slug>-<id>** · the next page → 0 cards   10:43 UTC
                                       **97 emitted, site states 97 — equal.**
GET /job-vacancies-south-sudan/logistics-administration-assistant-juba-118421                  200 — a JobPosting in JSON-LD (datePosted 2026-09-11T15:55:01+02:00), the JSON rejected by a strict parser (control characters) — read tolerantly, as `ghanajob.md` says
```

**The procedure is `ghanajob.md`'s six steps with this host's two paths**
— `/job-vacancies-search-south-sudan` for the listing, `/job-vacancies-south-sudan/<slug>-<id>` for the
