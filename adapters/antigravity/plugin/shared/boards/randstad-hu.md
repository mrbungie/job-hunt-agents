# Board adapter — Randstad Hungary (`www.randstad.hu`): the French platform's front by `randstadfr.py --host hu` — 364 advertisements in the board's own-language job-detail files on 2026-09-14 (363 public, 1 internal), the English copies set aside; the agency as employer on every ad; the consultant the description names withheld

<!-- verified: 2026-09-14 -->

<!-- hosts: www.randstad.hu -->
<!-- script: randstadfr.py -->
<!-- host-forms: www.randstad.hu -->
<!-- host-forms-basis: read — the sitemap index and every row are on `www.randstad.hu`; `randstadfr.py` names the host in `BOARDS["hu"]` and rewires its index on it before any request · 2026-09-14 -->
<!-- countries: HU -->
<!-- content: measured · **`/sitemaps/sitemap.xml` (200) names 35 files: 16 in Hungarian under sitemaps/jobs/hu/ and 16 in English under sitemaps/jobs/en/ (jobdetails, jobdetails-internal, joblistings and 13 sector listings each); the board's own-language `sitemap-jobdetails.xml` carries 363 `/allasok/<slug>_<town>_<n>/` rows (279 distinct lastmod for 363 rows) and `sitemap-jobdetails-internal.xml` 1 under `/allasok/karrier-a-randstadnal/` — 364 read as the inventory; the `/en/` files carry the same ads again and are set aside and said** — read by the declared client, the guard on the exact path (the rules, `*` open on the sitemaps and the ads, name the index); the ad (200, ~300 KB) carries the platform's JobPosting — `hiringOrganization` Randstad, `identifier.value` `38580`, locality and region, `employmentType`, `baseSalary` «600 HUF MONTH» minimum on the sample — the site's own figure, emitted as written, `datePosted`, `validThrough`, `industry`; **the page and the description name the consultant with e-mail and telephone — the description is scrubbed, the page's contact block never read** · 2026-09-14 -->
...
**Randstad Hungary is the staffing network's national front on Randstad's global
platform — the very stack `randstadfr.py` reads for France: one script,
`--host hu`.** Issue #363. Measured 2026-09-14 02:5x UTC by the
declared client, the guard on the exact path before each request.
Beside Profession.hu (`profession.md`), Állásportál (`allasportal.md`), CVOnline Hungary (`cvonline-hu.md`), it is the interim
network's own inventory, the client employer never named.

## The same platform, named per board

```
www.randstad.hu/robots.txt                          200 — the platform's file; Sitemap: /sitemaps/sitemap.xml
GET https://www.randstad.hu/sitemaps/sitemap.xml    200 — 35 files: 16 in Hungarian under sitemaps/jobs/hu/ and 16 in English under sitemaps/jobs/en/ (jobdetails, jobdetails-internal, joblistings and 13 sector listings each)
GET https://www.randstad.hu/sitemaps/jobs/hu/sitemap-jobdetails.xml            200 — 363 rows /allasok/<slug>_<town>_<n>/
GET https://www.randstad.hu/sitemaps/jobs/hu/sitemap-jobdetails-internal.xml   200 — 1 row(s) /allasok/karrier-a-randstadnal/… — Randstad's own vacancies
GET https://www.randstad.hu/sitemaps/jobs/en/sitemap-jobdetails.xml              the same 363 ads in English — set aside, never read twice
GET https://www.randstad.hu/allasok/targoncavezeto_csongrad_38580/   200 — <script type='application/ld+json'> JobPosting: hiringOrganization Randstad, identifier 38580, baseSalary in HUF
```
...
```
randstadfr.py search --host hu --limit 3
[randstad-hu] 2 job-detail file(s) in another language set aside — the same ads twice: en/sitemap-jobdetails-internal.xml, en/sitemap-jobdetails.xml
[randstad-hu] 364 ads in the sitemaps
[randstad-hu] 3 ads returned, 3 read
```
...
| `host` | yes | `hu` (or `www.randstad.hu`) |
...
`discover`: 364 rows; `search --limit 3`: three ads read, the consultant's
